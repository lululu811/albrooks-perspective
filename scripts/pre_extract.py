...
def parse_agent_file(filepath: Path, agent_name: str) -> SourceStats:
    ...
    # 统计来源数量：优先解析“来源统计”元数据块，其次回退到URL/引用数量
    stats.source_count = parse_source_count(content)
    if stats.source_count == 0:
        url_pattern = r'https?://[^\s\)\"\'\>]+'
        urls = re.findall(url_pattern, content)
        stats.source_count = len(urls) if urls else count_references(content)

    # 一手/二手/推测：优先使用“一手来源占比”字段
    ratio = parse_primary_ratio(content)
    if stats.source_count > 0 and ratio is not None:
        stats.primary_count = int(round(stats.source_count * ratio))
        stats.secondary_count = stats.source_count - stats.primary_count
        stats.inferred_count = 0
    else:
        # 回退：基于关键词粗估
    ...
    # 检测矛盾点：只从“矛盾与张力/矛盾标记/轻微矛盾”等小节提取，避免把普通“但/然而”误判为矛盾
    stats.contradictions = extract_section_lines(
        content,
        headers=["矛盾与张力", "矛盾标记", "轻微矛盾", "矛盾"],
        max_items=3,
    )

    # 检测缺口维度：从“缺口与不足/缺口/缺少/不足”等小节提取
    stats.gaps = extract_section_lines(
        content,
        headers=["缺口与不足", "缺口", "缺少", "不足"],
        max_items=3,
    )

    # 评估过程密度：优先读取“过程密度”标注，其次按推理关键词回退
    density = parse_process_density(content)
    if density:
        stats.process_density = density
    else:
...
def parse_source_count(content: str) -> int:
    """从“来源统计”元数据块中解析来源总数"""
    source_patterns = [
        r'书籍[：:]\s*(\d+)',
        r'著作[：:]\s*(\d+)',
    ...
    return sum(
        sum(int(m) for m in re.findall(pattern, content))
        for pattern in source_patterns
    )


def parse_primary_ratio(content: str) -> float | None:
    """解析“一手来源占比：80%”字段"""
    match = re.search(r'一手来源占比[：:]\s*(\d+)%', content)
    if match:
        return int(match.group(1)) / 100
    return None


def extract_section_lines(content: str, headers: list[str], max_items: int) -> list[str]:
    """提取指定标题（## 或 ###）下的列表项/句子，作为该维度的证据"""
    ...
    for header in headers:
        ...
        for i, line in enumerate(lines):
            if re.match(r'#{1,4}\s*' + re.escape(header), line.strip()):
                # 收集标题后的非空、非标题行，直到下一个同级或更高级标题
...
def parse_process_density(content: str) -> str | None:
    """从“过程密度”标注中解析 高/中/低"""
    for line in content.splitlines():
        if "过程密度" in line:
            if re.search(r'高|⭐{3,}', line):
                return "高"
            if re.search(r'中', line):
                return "中"
            if re.search(r'低', line):
                return "低"
    return None
...
def calculate_summary(report: PreExtractReport):
    ...
    if "Agent6_时间线" in report.agent_stats:
        ...
        # 提取年份：优先从完整内容中提取，而不是仅从标题
        years = re.findall(r'\b(19\d{2}|20\d{2})\b', timeline_stats.content)
        if years:
            report.time_span = (min(years), max(years))

    # 信号强度统计：从所有 Agent 内容中统计“强/中/弱信号”出现次数
    ...
    for stats in report.agent_stats.values():
        report.signal_strength["强信号"] += stats.content.count("强信号")
        report.signal_strength["中信号"] += stats.content.count("中信号")
        report.signal_strength["弱信号"] += stats.content.count("弱信号")
