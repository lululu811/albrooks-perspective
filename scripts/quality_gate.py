...
def parse_stats_file(file_path: Path) -> AgentStats:
    ...
    # 检查置信度标注：兼容“置信度”“来源可信度”“证据类型”等写法
    # 标题存在即可视为有标注区域；同时兼容标题与 emoji 不在同一行的写法
    stats.has_confidence_labels = bool(
        re.search(r'##\s*置信度标注|置信度标注|来源可信度|证据类型', content)
        or re.search(r'置信度[^\n]*(?:🟢|🟡|🔴|高|中|低)', content)
        or re.search(r'来源可信度[^\n]*(?:🟢|🟡|🔴|高|中|低)', content)
        or re.search(r'证据类型[^\n]*(?:强信号|中信号|弱信号|🟢|🟡|🔴)', content)
    )

    # 检查矛盾标注：兼容“矛盾与张力”“矛盾标记”“轻微矛盾”等写法
    stats.has_contradictions = bool(re.search(r'矛盾与张力|矛盾标记|轻微矛盾|矛盾', content))

    # 检查缺口标注：兼容“缺口与不足”“缺口”“缺少”“不足”等写法
    stats.has_gaps = bool(re.search(r'缺口与不足|缺口|缺少|不足|待调研', content))

    # 统计过程密度：兼容“过程密度：高”“**过程密度**：⭐⭐⭐⭐⭐ 最高”等写法
    density_blocks = re.findall(r'[^\n]*过程密度[^\n]*', content)
    for block in density_blocks:
        if re.search(r'(?:高|⭐{3,})', block):
            stats.process_density_high += 1
        if re.search(r'中', block):
            stats.process_density_medium += 1
        if re.search(r'低', block):
            stats.process_density_low += 1
...
def check_process_density(stats: AgentStats) -> QualityCheck:
    ...
    return QualityCheck(
        ...
        passed=total > 0 and high_ratio >= 0.3,
        actual=f"高:{stats.process_density_high} 中:{stats.process_density_medium} 低:{stats.process_density_low}",
        expected="高密度占比≥30%",
        suggestion="补充'过程密度'标注（高/中/低）及被追问'为什么'的完整推理链" if total == 0 or high_ratio < 0.3 else ""
    )
